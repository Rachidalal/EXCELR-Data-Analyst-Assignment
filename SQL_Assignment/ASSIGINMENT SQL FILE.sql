...
update countries set capitals='New Delhi'
where name='India';
...
values
( 'Laptop','A laptop computer, sometimes called a notebook computer by manufacturers, is a battery- or AC-powered personal computer',Null),
...
insert into employee values(100,'henry','jack',10,2000);
insert into employee values(200,'david','warner', 10, 3000),(300,'sachin','tendlukar',20,7500),(400,'wisky','wine',30,750);      
insert into employee values(300,'Johnny','walker', 20, 1300),(600,'rum','jin',30,850),(700,'era','fasak',30,75),(800,'George','reddy',10,10000);
insert into employee values(400,'amazon',Null, 10, 3000),(1000,'Gregory','rum',20,3500);
insert into employee values(500,'Jain','lomen',10,2700);
...
select count(distinct city) as 'total distinct'
from station;

select count(city)-count(distinct city) as 'difference between total number of city'
from station;
...
select  Orders.CUSTOMERNUMBER,customername,ordernumber,status,comments
FROM (CUSTOMERS
INNER JOIN ORDERS ON Orders.customerNumber = Customers.customerNumber)where status = 'disputed';


######### ASSIGNMENT________PART 3 #########
...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `assignment2`()
BEGIN
...
declare mycur cursor for select ordernumber,customernumber,comments from
...
open mycur;
 ...
 insert into cancellation (ordernumber,customernumber,comments)

values(lcl_orn,lcl_cn,lcl_com);
...
-- 3a) Write function that takes the customernumber as input and returns the purchase_status based on the following criteria . [table:Payments]
DELIMITER $$
CREATE DEFINER=`root`@`localhost` FUNCTION `STATUS_function`(CUST_NUM INT) RETURNS varchar(20) CHARSET utf8mb4
    DETERMINISTIC
BEGIN

 DECLARE my_status VARCHAR(20);

 declare amt int;

 select sum(amount) into amt from payments where customernumber=CUST_num;

 IF amt < 25000 THEN

 SET MY_STATUS='SILVER';
...
-- 3b) Write a query that displays customerNumber, customername and purchase_status from customers table.
SELECT customerNumber, customername,STATUS_FUNCTION(CUSTOMERNUMBER) AS MY_STATUS FROM CUSTOMERS;
...
Create table movies_archive(id integer, title varchar(50), category varchar(25));
...
SET ERRORMESSAGE = concat(' UPDATED MOVIES ');

						

IF NEW.id > 15 THEN

      SIGNAL SQLSTATE '45000'-- THIS IS HOW TO RAISE EXEMPTION ( TO READ ANY ISSUES)
...
UPDATE MOVIES SET id = 18 WHERE ID = 2;
-- ******************DELETE_TRIGGER*************************

delimiter $$

create trigger denied_deleting_movies
...
INSERT INTO  movies_archive(id,title,category)
...
select * from movies_archive;
-- -- **********************************
create table rentals(memid integer, first_name varchar(25), last_name varchar(25), movieid integer);
create table rentals_archive(memid integer, first_name varchar(25), last_name varchar(25), movieid integer);
...
create trigger denied_deleting_rentals
...
INSERT INTO  rentals_archive(memid,first_name, last_name,movieid)
...
delimiter ;
delete from rentals where memid=2;
select * from rentals_archive;

-- -- ****** ----UPDATE TRIGGER------******

DELIMITER $$
create trigger RENTALS_UPDATE
...
SET ERRORMESSAGE = concat(' updated rentals ');

						

IF NEW.memid > 15 THEN
...
UPDATE rentals SET memid = 18 WHERE memID = 2;

-- 5) Select the first name of the employee who gets the third highest salary. [table: employee]
select empid,fname,salary from employee order by salary desc;
